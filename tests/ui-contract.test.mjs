...
test('OTT home and navigation use the refreshed UI structure', () => {
  ...
  assert.match(manage, /<ProfileAuditPanel data=\{data\} manualMembers=\{manualMembers\} \/>/);
  ...
  assert.doesNotMatch(manage, /관리자 전용 비밀번호·PIN/);
  assert.doesNotMatch(manage, /계정 클릭 시에만 표시/);
  ...
  assert.doesNotMatch(manage, /credential\.password/);
  assert.doesNotMatch(manage, /credential\.pin/);
